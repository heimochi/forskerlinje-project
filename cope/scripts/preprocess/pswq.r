...
# Created: 2025-08-22
# Description: Loads raw PSWQ assessment data 
# ---------------------------------------------------------
...
pswq_core = rowSums(
  across(c(Q1, Q2, Q3, Q8, Q10, Q11, Q15, Q16)),
  na.rm = TRUE
),
pswq_core_aa = rowSums(
  !is.na(across(c(Q1, Q2, Q3, Q8, Q10, Q11, Q15, Q16)))
),
pswq_core_prorated = if_else(
  pswq_core_aa / 8 >= 0.70,
  score_prorate(pswq_core, pswq_core_aa, n_total = 8),
  NA_real_
),

# Uncontrollability of Worry (7 items)
pswq_uncont = rowSums(
  across(c(Q4, Q5, Q6, Q7, Q12, Q13, Q14)),
  na.rm = TRUE
),
pswq_uncont_aa = rowSums(
  !is.na(across(c(Q4, Q5, Q6, Q7, Q12, Q13, Q14)))
),
pswq_uncont_prorated = if_else(
  pswq_uncont_aa / 7 >= 0.70,
  score_prorate(pswq_uncont, pswq_uncont_aa, n_total = 7),
  NA_real_
),
...
pswq_engage_prorated = if_else(
  pswq_engage_aa / 1 >= 0.70,
  score_prorate(pswq_engage, pswq_engage_aa, n_total = 1),
  NA_real_
),

# Total (16 items, already reverse coded where needed)
pswq_total_prorated = if_else(
  pswq_total_aa / 16 >= 0.70,
  score_prorate(
    rowSums(across(Q1:Q16), na.rm = TRUE),
    pswq_total_aa,
    n_total = 16
  ),
  NA_real_
